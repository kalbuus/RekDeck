...
class DeckButton(Widget):
    def on_hue(self, instance, value):
        ...
        try:
            ...
            if area:
                # метод save_buttons_to_json может отсутствовать в упрощённой версии
                if hasattr(area, 'save_buttons_to_json'):
                    area.save_buttons_to_json()
        except Exception:
            pass
    ...
    def on_touch_down(self, touch):
        if self.collide_point(*touch.pos):
            # animate to pressed color on click
            try:
